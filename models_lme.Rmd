...
# formatting the results, starting with a function

format_moran <- function(moran) {
  tibble(I = moran$statistic,
         p = moran$p.value)
}

# putting Moran results in a nice table

do.call(rbind, lapply(moran_results, format_moran)) %>% 
  mutate(year = rep(2010:2019, times = 3),
         var = rep(c('rate', 'ie', 'late-early'), each = 10),
         # results = paste0(round(I, 3), ' (p=', round(p, 3), ')'),
         significant = p < 0.05) %>% 
  select(year, I, p) %>% 
  kable(booktabs = TRUE,
        #format = 'latex',
  ...
  row_spec(c(7, 11, 17, 27), bold = T, color = "black", background = rep("lightyellow"))
...
```{r display_category_coefficients}

# getting df for category coefficients

mutate(rownames_to_column(as.data.frame(summary(fitted_models$rate$category$best_mod)$tTable), 'term'), 
       dep = 'rate') %>% 
  bind_rows(mutate(rownames_to_column(as.data.frame(summary(fitted_models$ie$category$best_mod)$tTable), 'term'), dep = 'ie'),
            mutate(rownames_to_column(as.data.frame(summary(fitted_models$late_early$category$best_mod)$tTable), 'term'), dep = 'late-early')) %>% 
  clean_names() %>% 
  mutate(ci = ifelse(dep != 'ie',
                     paste0('(', round(value-qnorm(0.975)*std_error, 3), 
                            ', ', round(value+qnorm(0.975)*std_error, 3), ')'),
  ...
  kable(booktabs = TRUE,
        ...
        col.names = names_spaced,
        caption = 'Fixed Effects for Linear Mixed Effects Models with Spatiotemporal Autocorrelation') %>% 
  pack_rows('Policy category (within-between)', 2, 9) %>%
  pack_rows('Control variables', 10, 15) %>% 
  column_spec(1:4, width = "10em") %>% 
  row_spec(c(4, 7:9), bold = T, color = "black", background = rep("lightyellow"))
