  ...
  untransformed <- usa@data %>% 
    ...
    mutate(transformed = FALSE,
           transformation = case_when(name == 'abortion_per_1k_births' ~ 'sqrt(x)',
                                      name == 'prop_nonres' ~ 'log(x)',
                                      name == 'prop_late' ~ 'sqrt(x)')) 
...
names_spaced <- c(
  ...
  'IE model: Multiplicative change in nonresident abortion rate',
  'Late-early model: Change in the square root of the later-term abortion rate'
  )
...
big_coef_df <- mutate(get_summary_df(rate_mod), dep = 'rate') %>% 
  bind_rows(mutate(get_summary_df(prop_nonres_mod), dep = 'ie'),
            mutate(get_summary_df(prop_late_mod), dep = 'late-early')) %>% 
  clean_names() %>% 
