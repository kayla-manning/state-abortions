...
# now I want to calculate a value to add to the policy score based on a
# state's harshness in each area

score_key <- policy_levels %>% 
  group_by(policy, category, category_weight) %>% 
...
# creating df of categories and weights

policy_categories <- score_key %>% 
  select(policy, category, category_weight) %>% 
  distinct() %>% 
  group_by(category) %>% 
  mutate(n_policies = n()) %>% 
  ungroup()
...
within_scores_df <- clean %>% 
  pivot_longer(licensed_physician:parental_involv_minors, 
               names_to = 'policy', values_to = 'level') %>% 
  inner_join(policy_categories, by = c('policy')) %>% 
  left_join(select(score_key, -c(category, category_weight)), 
            by = c('policy', 'level')) %>% 
    mutate(score = case_when(level %in% c('0', 'Enjoined') ~ 0,
                             TRUE ~ score))  %>% 
  group_by(state, year, category, category_weight, n_policies) %>%
  summarize(hostility_score = sum(score),
            .groups = 'drop') %>%
  arrange(desc(hostility_score)) %>% 
  mutate(rescaled_score = hostility_score / n_policies * category_weight) %>% 
  group_by(state, year) %>% 
  summarise(hostility_score = sum(rescaled_score),
            .groups = 'drop')
...
write_csv(all_scores, 'data-creation/raw-data/reweighted_policy_scores.csv')
