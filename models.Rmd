...
pred_diff_df <- dummy_df %>% 
  select(within_score, surrounding_score) %>% 
  mutate(pred = c(predict(rate_mod, base_df)^2,
                   exp(predict(rate_nonres_mod, base_df)),
                   predict(rate_res_mod, base_df)^2,
                   predict(rate_late_mod, base_df)^2,
                   predict(rate_early_mod, base_df)^2,
                   predict(prop_late_mod, base_df)^2),
...
# creating similar df but this time with Kansas's demographic variables for 2019

ks_dummies <- tibble(surrounding_score = rep(dummy_scores, times = 3),
                     within_score = rep(seq(-1, 1), each = length(dummy_scores)),
                     prop_bachelors = usa@data$prop_bachelors[usa@data$NAME_1=='Kansas' & 
                                                                usa@data$year == 2019],
                     prop_hisp = usa@data$prop_hisp[usa@data$NAME_1=='Kansas' & 
                                                      usa@data$year == 2019],
                     prop_nonwhite = usa@data$prop_nonwhite[usa@data$NAME_1=='Kansas' & 
                                                                usa@data$year == 2019],
                     hh_income = usa@data$hh_income[usa@data$NAME_1=='Kansas' & 
                                                                usa@data$year == 2019],
                     dem_2party = usa@data$dem_2party[usa@data$NAME_1=='Kansas' & 
                                                                usa@data$year == 2019],
                     total_population = usa@data$total_population[usa@data$NAME_1=='Kansas' & 
                                                                    usa@data$year == 2019],
                     year = 2019)

# now generating the predictions

ks_predicts <- ks_dummies %>% 
  select(within_score, surrounding_score) %>% 
  mutate(rate = predict(rate_mod, ks_dummies)^2,
         rate_nonres = exp(predict(rate_nonres_mod, ks_dummies)),
         rate_res = predict(rate_res_mod, ks_dummies)^2,
         rate_late = predict(rate_late_mod, ks_dummies)^2,
         rate_early = predict(rate_early_mod, ks_dummies)^2,
         prop_late = predict(prop_late_mod, ks_dummies)^2) %>% 
...
# visualizing all outcomes together... in 2019 KS had within score of 1.8 and surrounding score of 1.2
...
usa@data$surrounding_score[usa@data$NAME_1=='Kansas' & usa@data$year == 2019]
