  ...
  comparison_df %>% 
    ...
    kable(booktabs = TRUE,
          col.names = c('Method', 'Inverse distance', 
                        ...
                        'Contiguous')) %>% 
    pack_rows('Abortion Rates (abortions per 1k live births)', 1, 5) %>% 
    pack_rows('Import-Export Ratio', 6, 10) %>% 
    pack_rows('Late-Early Ratio', 11, 15) %>% 
    add_header_above(header = c(' ' = 1, 'AIC for neighborhood specification' = 3)) %>% 
    row_spec(c(1, 6, 11), bold = T, color = "black", background = "lightyellow")
}
