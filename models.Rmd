...

{
  ...
  facet_labels <- c('Abortion rates', 'Nonresident abortion rate', 'Resident abortion rate',
  ...
  ggarrange(plots[[4]], plots[[5]], plots[[6]],
            nrow = 3)
}
...
usa@data %>% 
  ...
  pivot_longer(everything()) %>% 
  mutate(name = str_replace(str_to_title(str_replace_all(name, '_', ' ' )), 'Score', 'score')) %>% 
  ggplot(aes(value)) +
  geom_histogram() +
  ...
  labs(x = 'Standardized score',
       y = 'Count',
       title = 'Standardized policy scores')
...
mutate(get_lme_summary_df(rate_mod), dep = 'rate') %>% 
  bind_rows(mutate(get_lme_summary_df(rate_nonres_mod), dep = 'nonres'),
            mutate(get_lme_summary_df(rate_res_mod))) %>% 
  mutate(ci = paste0('(', round(value-qnorm(0.975)*std_error, 3), 
                     ', ', round(value+qnorm(0.975)*std_error, 3), ')'),
         value = round(value, 5)) %>% 
  mutate(value = ifelse(p_value < 0.05, paste0(value, '*'), value),
  ...
  kable(booktabs = TRUE,
        #format = 'latex',
        col.names = rate_colnames,
        caption = 'Fixed-Effects Coefficients for Linear Mixed-Effects Rate Models',
        align = c('l', rep('c', times = 3))) %>% 
  pack_rows('Policy environment', 2, 4) %>%
  pack_rows('Control variables', 5, 10) %>% 
  column_spec(1:4, width = "15em") %>% 
...
bind_rows(mutate(get_lme_summary_df(rate_mod), dep = 'rate') %>% 
  ...
  kable(booktabs = TRUE,
        #format = 'latex',
        col.names = time_colnames,
        caption = 'Fixed-Effects Coefficients for Linear Mixed-Effects Timing Models',
        align = c('l', rep('c', times = 3))) %>% 
  pack_rows('Policy environment', 2, 4) %>%
  pack_rows('Control variables', 5, 10) %>% 
  column_spec(2:4, width = "40em") %>% 
  row_spec(c(2, 4), bold = T, color = "black", background = "lightyellow")
...
# now I want to create a formatted table **IN THE MIDDLE OF THIS WHEN I STOPPED**
...
all_robustness_df  %>% 
  ...
  pack_rows('Nonresident abortion rate', 4, 6) %>% 
  pack_rows('Resident abortion rate', 7, 9) %>% 
  pack_rows('Later-term abortion rate', 10, 12) %>% 
  pack_rows('Earlier-term abortion rate', 13, 15) %>% 
  pack_rows('Later-term share of abortions', 16, 18) %>% 
