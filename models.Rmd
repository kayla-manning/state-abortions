...
moran_outcomes %>% 
  ...
  mutate(across(statistic:p.value, function(x) round(x, 3)),
         statistic = ifelse(p.value < 0.05, paste0(statistic, '*'), statistic)) %>% 
  kable(booktabs = TRUE,
        #format = 'latex',
        col.names = c('Year', 'I-statistic', 'p-value'),
        caption = "Moran's I-test on outcome variables",
        align = rep('c', 3)) %>% 
  pack_rows('Overall Abortion Rate', 1, 10) %>%
...

{
  ...
  ggsave('figures/transform_dists.png', width = 9, height = 7)
}
...
usa@data %>% 
  ...
  labs(x = 'Standardized Policy Score',
       y = 'Count')
ggsave('figures/std_score_hist.png',
       width = 5, height = 4)
...
display_rate_df %>% 
  ...
  kable(booktabs = TRUE,
        ...
        col.names = c('Variable', 'Overall Rates', 'Nonresident Rates', 'Resident Rates',
                      'Later-Term Rates', 'Earlier-Term Rates', 'Later-Term Share'),
        caption = 'Fixed-Effects Coefficients for Linear Mixed-Effects Models of Transformed Outcomes',
...
base_df <- tibble(surrounding_score = rep(dummy_scores, times = 3),
                   ...
                   total_population = 0,
                   year = 2019)
dummy_df <- bind_rows(base_df, base_df, base_df, 
                      base_df, base_df, base_df)

# writing a function to get error bands around predictions for nlme models

get_se <- function(mod) {
  
  #create design matrix
  
  Designmat <- model.matrix(eval(eval(mod$call$fixed)[-2]), base_df[-ncol(base_df)])
...
pred_diff_df <- dummy_df %>% 
  select(within_score, surrounding_score) %>% 
  mutate(pred = c(predict(rate_mod, base_df),
                   predict(rate_nonres_mod, base_df),
                   predict(rate_res_mod, base_df),
                   predict(rate_late_mod, base_df),
                   predict(rate_early_mod, base_df),
                   predict(prop_late_mod, base_df)),
         se = c(get_se(rate_mod),
                get_se(rate_nonres_mod),
                get_se(rate_res_mod),
                get_se(rate_late_mod),
                get_se(rate_early_mod),
                get_se(prop_late_mod)),
         outcome = rep(c('rate', 'rate_nonres', 
                         'rate_res', 'rate_late', 
                         'rate_early', 'prop_late'),
                       each = nrow(base_df))) %>% 
...
pred_diff_df %>% 
  mutate(ymin = case_when(!str_detect(outcome, 'Nonresident') ~ (pred-2*se)^2,
                          TRUE ~ exp(pred-2*se)),
         ymax = case_when(!str_detect(outcome, 'Nonresident') ~ (pred+2*se)^2,
                          TRUE ~ exp(pred+2*se))) %>% 
  mutate(pred = ifelse(!str_detect(outcome, 'Nonresident'), pred^2, exp(pred))) %>% 
  ggplot(aes(surrounding_score, pred, color = as.factor(within_score),
             fill = as.factor(within_score))) +
  geom_line() +
...
ggsave('figures/all_synthetic.png',
       width = 7, height = 4)
...
# writing a function to produce model residual map for a specified year

get_resid_map <- function(mod, year, var) {
  
  resids <- case_when(var == 'overall' ~ 
                        mod$fitted[,1]^2 - usa@data$abortion_per_1k_births,
                      var == 'early' ~
                        mod$fitted[,1]^2 - usa@data$rate_early,
                      var == 'res' ~
                        mod$fitted[,1]^2 - usa@data$rate_res,
                      var == 'nonres' ~
                        exp(mod$fitted[,1]) - usa@data$rate_nonres)
  usa@data %>% 
    ...
    filter(year == year) %>% 
    ggplot() +
    geom_polygon(aes(long, lat, group=group, fill = resids),
                 color = 'black') +
...
# significant Moran results in...
# - overall rate: 2012
# - nonresident rate: 2010, 2017
# - resident rate: 2012
# - earlier-term rate: 2012-2015

p1 <- get_resid_map(rate_mod, 2012, var = 'overall') +
  labs(fill = 'Overall Rate Residuals, 2012')
p2 <- get_resid_map(rate_nonres_mod, 2010, var = 'nonres') +
  labs(fill = 'Nonresident Rate Residuals, 2010')
p3 <- get_resid_map(rate_res_mod, 2012, var = 'res') +
  labs(fill = 'Resident Rate Residuals, 2012')
p4 <- get_resid_map(rate_early_mod, 2012, var = 'early') +
  labs(fill = 'Earlier-Term Rate Residuals, 2012')
ggarrange(p1, p3, p4, p2, nrow = 2, ncol = 2)
ggsave('figures/resid_maps.png',
       width = 9,
       height = 7)
...
for (i in 1:length(models)) {
  ...
  path <- file.path(paste0('figures/', abbrev[i], '_diagnostics.png'))
  ...
  plots[[i]] <- ggarrange(resid_vs_fit, qqplot)
  plots[[i]]
