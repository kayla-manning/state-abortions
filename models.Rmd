...
rate_wsar <- spautolm(abortion_per_1k_births ~ within_between + pct_bachelors + prop_hisp + 
               ...
               family = 'SAR',
               listw = weights_matrix)
models <- list(rate_lm, rate_lmw, rate_car, rate_wcar, rate_wsar)
...
for (mod in models) {
  ...
  if (class(mod) == 'lm') {
    print(lm.morantest(mod, listw = weights_matrix, alternative = 'two.sided'))
  }
  ...
  # evidence of additional autocorrelation not captured by the model
    ...
    print(moran.test(residuals(mod), listw = weights_matrix, alternative = 'two.sided'))
  }
}
