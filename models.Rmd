...
source('model_helper_raw_scores.R')
# source('fit_models.R')
...
# moran test using the row-normalized contiguous neighbor weights, inverse
# weighted distance weights, & distance squared weights
...
# same thing but for import-export

{
  moran.mc(usa@data$ie_ratio,
         listw = weights.contig.W,
         nsim = 9999,
         alternative = 'two.sided')
  moran.mc(usa@data$ie_ratio,
...
# same thing but for late-early

{
  moran.mc(usa@data$late_to_early,
         listw = weights.contig.W,
         nsim = 9999,
         alternative = 'two.sided')
  moran.mc(usa@data$late_to_early,
...
late_early_sar <- late_early_inv_dist[[2]][6][[1]]
summary(late_early_sar)

# printing moran test.. both this & the LR in the model summary have p-values <
# 0.05... figure out what that means again

late_early_inv_dist[[3]][6][[1]]

# printing diagnostic plots

late_early_inv_dist2[[4]][6][[1]]
...
big_coef_df <- mutate(get_summary_df(rate_sar), dep = 'rate') %>% 
  ...
  mutate(term = fct_relevel(term,
                            ...
                            'interstate_score', 
                            'intrastate_score', 
                            'interstate_score:intrastate_score',
...
big_coef_df %>% 
  ...
  pack_rows('Policy environment', 2, 3) %>%
  pack_rows('Control variables', 4, 9) %>% 
  pack_rows('Year', 10, 18) %>% 
  column_spec(1:4, width = "10em") %>% 
  row_spec(2, bold = T, color = "black", background = "lightyellow")
...
cor(usa@data$interstate_score,
    usa@data$intrastate_score)
...
mutate(get_model_comparisons(rates_inv_dist, rates_inv_dist2, rates_contig), var = 'rate') %>% 
  ...
  kable(booktabs = TRUE) %>% 
  pack_rows('Abortion Rates (abortions per 1k live births)', 13, 18) %>% 
  pack_rows('Import-Export Ratio', 1, 6) %>% 
  pack_rows('Late-Early Ratio', 7, 12) %>% 
  add_header_above(header = c(' ' = 1, 'AIC' = 3, ' ' = 2))
