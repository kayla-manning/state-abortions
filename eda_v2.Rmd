...
ggarrange(within_all, within_nonmissing,
  ...
  annotate_figure(left = text_grob('Count', rot = 90, vjust = 1, 
                                  size = 12),
                  top = text_grob('Within-state policy scores', size = 14))
...
ggarrange(surrounding_all, surrounding_nonmissing,
  ...
  annotate_figure(left = text_grob('Count', rot = 90, vjust = 1, 
                                  size = 12),
                  top = text_grob('Surrounding-state policy scores', size = 14))
...
inner_join(reporting_terciles, all_terciles) %>% 
  ...
  row_spec(3, bold = T, color = "black", background = rep("lightyellow")) %>% 
...
# visualizing on 2d plot (scatterplot with intrastate on x and interstate on y)
{
  combined %>% 
    ...
    geom_hline(data = filter(pivot_longer(combined, within_score:surrounding_score),
               ...
               color = 'red') +
    theme_minimal() +
    labs(title = 'Visualizing policy terciles',
         color = '[Within]-[surrounding]',
...
state_nonreport_cats %>% 
  ...
  scale_fill_manual(breaks = c('All', 'Some', 'None'),
                    values = c(crimson, 'indianred1', 'ivory4')) +
  coord_fixed() +
...
asofdf %>% 
  ...
  row_spec(which(!str_detect(asofdf$as_of, 'January')),
           bold = T, color = "black", background = rep("lightyellow"))
