...

{
  ...
  # cleans Guttmacher's policy tables in the formatting used for 2017-2022...
  # will use this for the scraped data below
...

{
  ...
  for (i in 1:nrow(sources)) {
    ...
    if (length(tables) != 0) {  
      ...
      # adding the cleaned data to an empty df with policies across all of the
      # years
...

{
  ...
  for (y in 2006:2016) {
    ...
    
    {
      ...
      if (y %in% c(2008, 2009, 2010)) {
        these_coords <- coords_df %>% 
          mutate(top = case_when(colname %in% c('instit_prov_refuse',
                                                'mandate_fetal_pain') ~ top-10,
                                 colname == 'hr_wait_post_counseling' ~ top-2.7,
                                 colname == 'mandate_breast_cancer' ~ top-25,
                                 TRUE ~ top),
                 bottom = case_when(colname %in% c('instit_prov_refuse',
                                                   'parental_involv_minors',
      ...
      if (y==2016) {
        these_coords <- coords_df %>% 
          mutate(top = case_when(colname %in% c('state') ~ top+5,
                 ...
                 right = case_when(colname == 'mandate_breast_cancer' ~ right-7,
                                   colname == 'indiv_prov_refuse' ~ right-5,
                                   colname == 'parental_involv_minors' ~ right+15,
                                   colname %in% c('mandate_fetal_pain',
                                                  'mandate_neg_psych') ~ right+30,
                                   TRUE ~ right))  
      }
    ...
    # the tables are split across two pages, and each page has different columns.
    # here, I go through one page at a time and extract the data
    ...
    for (p in 1:2) {
      ...
      for (i in 1:length(colnames_list[[p]])) {
        ...
        # cleaning the data from that single column and storing it in an empty vector
        
        clean_col <- rep(NA, nrow(test))
        for (r in 1:nrow(test)) {
          
          # it splits each column of data into multiple columns, so I am combining
          # values in the same rome into a single column and removing any
        ...
        # assigning the vector of values to the corresponding column name
    ...
    # cleaning the data from both pages (for that single year) once I have all of
    # the columns read in
    
    {
      # hard-coding in the states because 2008-2010 state columns weren't reading in
      # correctly
      ...
      # once I have named vectors for each column of values, I will bind them all
      # together and convert to a df
      ...
      messy_df <- results_matrix[,!duplicated_cols] %>% 
        as.data.frame() %>% 
        mutate(as_of = sources$as_of_date[sapply(sources$links, 
                                                 function(x) str_detect(x, 
                                                                        as.character(paste0('/', y))))],
               accessed = sources$date_accessed[sapply(sources$links, 
                                                       function(x) str_detect(x, 
                                                                              as.character(paste0('/', y))))])
      
      # once I have this year's data, I am adding it to the df that I am using to
      # store the data for all years
  ...
  # adding a year column to the policy df to make it easier to read & making a few
  # manual tweaks that could not be fixed by adjusting the coordinates
  
  policy_df <- policy_df %>% 
    mutate(year = year(as_of)) %>% 
    select(state, year, everything()) %>% 
    arrange(year) %>% 
    mutate(hr_wait_post_counseling = case_when(state == 'AL' & year %in% c(2008:2014) ~ '24',
                                               state == 'AL' & year %in% 2015:2016 ~ '48',
                                               TRUE ~ hr_wait_post_counseling),
           parental_involv_minors = ifelse(state == 'AL' & year == 2016, 'Consent', 
                                           parental_involv_minors))
